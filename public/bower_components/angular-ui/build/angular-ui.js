...
angular.module('ui.directives').directive('uiSpinner', ['ui.config',
    function(uiConfig) {
        'use strict';
        ...
        return {
            ...
            link: function(scope, $element, attrs, controller) {
                function updateModel() {
                    controller.$setViewValue($element.spinner('value'));
                }
                ...
                function initWidget(options) {
                    ...
                    if (controller) {
                        ...
                        if (options.stop) {
                            // caller has specified stop, so call this as well as updating the model
                            var userHandler = options.stop;
                            options.stop = function(value, spinner) {
                                scope.$apply(function() {
                                    updateModel();
                            ...
                            // no stop already specified so just update the model
 ...
 */
angular.module('ui.directives').directive('uiSelect2', ['ui.config', '$timeout',
    function(uiConfig, $timeout) {
        var options = {};
        if (uiConfig.select2) {
